...
// Convención de ruta <salon_id>/<uuid>.<ext>, igual que service-images.ts —
// es la que leen las políticas RLS de storage.objects (migración 0017).
...
export async function deleteSalonLogo(supabase: SupabaseServerClient, logoUrl: string) {
...
function extractStoragePath(logoUrl: string): string | null {
  ...
  return decodeURIComponent(logoUrl.slice(index + marker.length));
}
